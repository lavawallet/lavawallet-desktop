...
module.exports =  class SocketServer {
  constructor( ){
    ...
    /*
              socket.on('createAccount', async function (data,fn) {
                      var acct = await AccountHelper.createAccount();
                      fn(JSON.stringify(acct));  //immediate response
                });

*/
      /*    socket.on('getNametagInfo', async function (data,fn) {
                  var acct = await AccountHelper.getNametagInfo(data);
                  fn(JSON.stringify(acct));  //immediate response
            });*/
              ...
              socket.on('getWalletInfo', async  function (data,fn) {
                ...
                socket.on('unlockAccount', async  function (data,fn) {

                        var info = await  AccountHelper.unlockAccount(data.address,data.password);
                        fn(info);  //immediate response
                  });



              socket.on('getNetworkInfo', async function (address,fn) {
                      var info = await NetworkHelper.getNetworkInfo( );
                      fn({success:true, networkInfo: info});  //immediate response
                });
